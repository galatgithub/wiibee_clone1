...
class Wiiboard:
    ...
    def calc_mass(self, raw, pos):
        ...
        if self.calibration[1][pos] - self.calibration[0][pos] <= 0.001: # update
            return 0 # update
        if self.calibration[2][pos] - self.calibration[1][pos] <= 0.001: # update
            return 0 # update
    ...
    def loop(self):
        ...
        while self.running and self.receivesocket:
            ...
            if input_type == INPUT_STATUS:
                ...
                self.battery = data[14] * 0.039315 + 0.0907875 # update
                self.temperature = data[12] # update

    def on_status(self):
        self.reporting() # Must set the reporting type after every status report
        logger.info(f"Status: battery: {self.battery} light: %s", # update
